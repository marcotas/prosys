# Step 5: Habit Entity Class

> Habit entity with private constructor, hybrid `create()`/`fromData()`, mutable mutations. Simpler than Task — no weekStart/dayIndex validation.

**Files:**
- Create: `src/lib/domain/habit.ts`
- Create: `src/lib/domain/habit.test.ts`

**Dependencies:** Step 1 (types with `HabitData`)
...
describe('Habit.create', () => {
	...
	it('throws on empty memberId', () => {
		expect(() => Habit.create({ ...validInput, memberId: '' })).toThrow(
			'Member ID is required'
		);
	});

	it('collects multiple validation errors', () => {
		expect(() => Habit.create({ memberId: '', name: '' })).toThrow(
			/Name is required.*Member ID is required/
		);
	});
});
...
Run: `pnpm test src/lib/domain/habit.test.ts`

Expected: FAIL — `habit.ts` does not exist.
...
```bash
git add src/lib/domain/habit.ts src/lib/domain/habit.test.ts
git commit -m "feat: add Habit entity class with tests"
```

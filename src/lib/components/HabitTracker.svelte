<script lang="ts">
	...
	let { habits, theme, onToggleHabit, onAddHabit, onUpdateHabit, onDeleteHabit } = $props<{
		habits: Habit[];
		theme: ThemeConfig;
		onToggleHabit: (habitId: string, dayIndex: number) => void;
		onAddHabit: (name: string, emoji?: string) => void;
		onUpdateHabit: (habitId: string, updates: { name?: string; emoji?: string }) => void;
	...
	function startEdit(habitId: string, currentName: string) {
		if (swipedOpenId) { swipedOpenId = null; return; }
...
>
	...
	{#if !collapsed}
		<div class="overflow-x-auto">
			<table class="w-full">
				<thead>
					<tr class="border-b" style="border-color: {playful ? `${theme.accent}12` : '#f3f4f6'}">
						<th
				...
				<tbody>
					{#each habits as habit (habit.id)}
						...
						{@const isSwiping = dragState?.habitId === habit.id && dragState?.locked}
						...
						>
							...
							<td class="relative overflow-hidden p-0">
								...
								<div
									class="relative bg-white px-4 py-2 flex items-center {isSwiping ? '' : 'transition-transform duration-200 ease-out'}"
									style="transform: translateX({offset}px); touch-action: pan-y; z-index: 1;"
									onpointerdown={(e) => onPointerDown(e, habit.id)}
								>
									{#if habit.emoji}<span class="mr-1 select-none" aria-hidden="true">{habit.emoji}</span>{/if}
									<input
										type="text"
										value={editingHabitId === habit.id ? editValue : habit.name}
										oninput={(e) => { editValue = e.currentTarget.value; }}
										onfocus={(e) => { startEdit(habit.id, habit.name); }}
										onblur={() => commitEdit(habit.id, habit.name)}
										onkeydown={(e) => {
											if (e.key === 'Enter') { e.preventDefault(); e.currentTarget.blur(); }
											if (e.key === 'Escape') { editingHabitId = null; editValue = ''; e.currentTarget.value = habit.name; e.currentTarget.blur(); }
										}}
										class="text-[13px] w-full bg-transparent border-none outline-none p-0 m-0 whitespace-nowrap {playful ? 'font-medium' : 'text-gray-700'}"
							...
							<td class="px-4 py-2 text-right">
								<div class="flex items-center justify-end gap-2">
									<div
										...
										style="background-color: {progress === 100 ? `${theme.accent}30` : playful ? `${theme.accent}15` : '#f0f0f0'}"
										role="progressbar"
									...
									{#if progress === 100}
										<span class="text-sm w-9 text-right" aria-label="100% complete">🏆</span>
									{:else}
										<span
					...
					<tr>
						<td colspan="9" class="px-4 py-2">
							<input
								...
								onkeydown={(e) => { if (e.key === 'Enter') { e.preventDefault(); submitNewHabit(); } else if (e.key === 'Escape') { newHabitName = ''; (e.target as HTMLInputElement).blur(); } }}
								class="text-[13px] w-full bg-transparent border-none outline-none p-0 m-0 placeholder:text-gray-300 {playful ? 'font-medium placeholder:opacity-60' : 'text-gray-700'}"
		...
		{#if !hasHabits}
			<div class="flex flex-col items-center justify-center py-6 text-center px-4">
				<span class="text-2xl mb-2" aria-hidden="true">{playful ? '🌈' : '📊'}</span>
				<p class="text-sm font-medium" style="color: {playful ? theme.accent : '#9ca3af'}">
